...
from src.file_processing import processing_function_csv, processing_function_excel
...
\t3. Получить информацию о транзакциях из XLSX-файла
    ...
    while True:
        ...
        if result == "1":
            ...
            lst_temp = function_accepts_json(os.path.join(os.path.dirname(__file__), "data", "operations.json"))
            for temp in lst_temp:
                dct = {"id": temp.get("id", 0),
                       "state": temp.get("state", ""),
                       "date": temp.get("date", ""),
                       "amount": temp.get("operationAmount", {}).get("amount", ""),
                       "currency_name": temp.get("operationAmount", {}).get("currency", {}).get("name", ""),
                       "currency_code": temp.get("operationAmount", {}).get("currency", {}).get("code", ""),
            ...
            lst_transaction = processing_function_csv(os.path.join(os.path.dirname(__file__), "data", "transactions.csv"))
            break
        elif result == "3":
            print("Для обработки выбран XLSX-файл.")
            lst_transaction = processing_function_excel(os.path.join(os.path.dirname(__file__), "data", "transactions_excel.xlsx"))
            break
        else:
...
Доступные для фильтровки статусы: EXECUTED, CANCELED, PENDING""")
    ...
    result = input().strip()
    if result.lower() == "да":
        lst_transaction = search_transactions(lst_transaction, input("Введите слово для поиска: ").strip())
