...
def get_mask_account(account_number: int) -> str:
    """Функцию маскировки номера банковского счета которая принимает счет и возвращает последние 4 цыфры
    и маскирует 2 перд ними"""
    text_number_account = str(account_number)  # преабразуем строку в текст
    mask_numbers_account = "**"  # пишем переменную для маскировки цыфр
    mask_account = f"{mask_numbers_account}{text_number_account[-5:-1]}"
    return mask_account
